...
layui.define(["layer", "laytpl", "upload"], function (i) {
	...
	H = function (i) {
		...
		create = function (a) {
			...
			L(n(y).render(s)),
			(l.close || i.min) && I(),
			layui.each(d.ready, function (i, a) {
				a && a(s)
			})
		},
		j = e.extend(j, obj),
		i.brief ? layui.each(d.ready, function (i, a) {
			a && a(obj)
	...
	T = function (i) {
		...
		var u = C = t.open({
				...
				success : function (a) {
					...
					P(),
					q(),
					layui.each(d.chatChange, function (i, a) {
						a && a(O())
					}),
	...
	D = function () {
		...
		if (i.content = a.textarea.val(), "" !== i.content.replace(/\s/g, "")) {
			...
			B(o),
			layui.each(d.sendMessage, function (i, a) {
				a && a(s)
			})
		}
		W(),
	...
	Y = function (i) {
		...
		t = {},
		l = a.index();
		if (i.timestamp = i.timestamp || (new Date).getTime(), i.fromid == j.mine.id && (i.mine = false), i.system || B(i), K = JSON.parse(JSON.stringify(i)), j.base.voice && G(), !w && i.content || l === -1) {
			if (j.message[i.type + i.id])
				...
				layui.each(j.friend, function (a, e) {
					if (layui.each(e.list, function (a, e) {
							if (e.id == i.id)
								return e.type = "friend", e.name = e.username, j.chat.push(e) , s = !0
							}), s)return !0
				}),
				...
				layui.each(j.group, function (a, e) {
					if (e.id == i.id)
						return e.type = "group", e.name = e.groupname, j.chat.push(e), o = !0
				}),
				o || (i.name = i.groupname, j.chat.push(i))
			} else
				i.name = i.name || i.username || i.groupname, j.chat.push(i);
	...
	U = function (i) {
		...
		if (!a) {
			...
			if ("friend" === i.type) {
				...
				"group" === i.type && (l.append(s), l.find(".layim-null")[0] && l.find(".layim-null").remove())
		}
	},
	V = function (i) {
		...
		j[i.type] && ("friend" === i.type ? layui.each(j.friend, function (e, t) {
				...
				if (i.id == t.id)
					return a.find(">li").eq(e).remove(), j.group.splice(e, 1), 0 === j.group.length && a.html('<li class="layim-null">暂无群组</li>'), !0
			}))
	},
	W = function () {
		var i = O(),
	...
	ei = {
		...
		sign : function () {
			...
			i.on("change", function () {
				var i = this.value;
				layui.each(d.sign, function (a, e) {
					e && e(i)
				})
			}),
				...
				if ("" === a)
					...
					for (var n = [], l = j.friend || [], s = j.group || [], o = "", r = 0; r < l.length; r++)
						for (var d = 0; d < (l[r].list || []).length; d++)
							l[r].list[d].username.indexOf(a) !== -1 && (l[r].list[d].type = "friend", l[r].list[d].index = r, l[r].list[d].list = d, n.push(l[r].list[d]));
			...
			i.data("down") ? l() : (n.html("&#xe619;"), i.data("down", !0), ei.groupMembers.index = t.tips('<ul class="layim-members-list"></ul>', i, {
						...
						success : function (a) {
							...
							S(t, function (a) {
								...
								layui.each(d.members, function (i, e) {
									e && e(a)
								})
							}),
		...
		setSend : function (i, a) {
			...
			n = i.attr("lay-type");
			if ("show" === n)
				X(a), t.show().addClass(ai), e(document).off("click", ei.setSendHide).on("click", ei.setSendHide);
			else {
				i.addClass(o).siblings().removeClass(o);
		...
		image : function (i) {
			...
			layui.upload({
				...
				type : a,
				success : function (i) {
					0 == i.code ? (i.data = i.data || {}, "images" === a ? ii(n.textarea[0], "img[" + (i.data.src || "") + "]") : "file" === a && ii(n.textarea[0], "file(" + (i.data.src || "") + ")[" + (i.data.name || "下载文件") + "]"), D()) : t.msg(i.msg || "上传失败")
				}
			})
		},
		extend : function (i) {
			...
			e = O();
			layui.each(d["tool(" + a + ")"], function (a, t) {
